...
int LookaheadPrefetcher::SPP_class::access_ST(int tag, int curr_block, int type)
{
	...
	for (i=0; i<ST_WAY; i++) {
		if (valid[i] && (p_tag[i] == tag)) { // Hit: Matching page
			...
			D(printf("\t[ST-UPDATE] Index: %4x ST-Hit[%d]: %3x (partial_pnum) curr_block: %d last_block[%d]: %d\n",
						idx, i, tag, curr_block, i, last_block[i]));

			// Update ST
			last_block[i] = curr_block;
			LookaheadPrefetcher::SPP_class::update_lru(i, false, FROM_ST);
			ST_hit++; ST_access++;
			return update_signature(i, type);
		}
			...
			p_tag[i] = tag;
			last_block[i] = curr_block;
			D(printf("\t[ST-UPDATE] Index: %4x ST-Invalid[%d]: %x (partial_pnum) curr_block: %d last_block[%d]: %d\n",
						idx, i, tag, curr_block, i, last_block[i]));
			delta[match_idx][HISTORY-1] = curr_block; // DEBUG
			LookaheadPrefetcher::SPP_class::update_lru(i, true, FROM_ST);
	...
	D(printf("\t[ST-UPDATE] Index: %4x ST-Miss[%d]: %x (partial_pnum) does not exist => Clear history and replacing lru[%d]: %d\n",
...
int LookaheadPrefetcher::SPP_class::update_signature(int assoc, int type)
{
	...
	if (type == UPDATE) { // Update signature
		...
		// DEBUG
		D(printf("\t[ST-UPDATE] Index: %4x curr-History[%d]: ", idx, assoc));
		for (int i=0; i<(HISTORY-1); i++) {
			delta[assoc][i] = delta[assoc][i+1];
			D(printf("%3d ", delta[assoc][i]));
		}
		delta[assoc][HISTORY-1] = temp_delta;
		D(printf("%3d", delta[assoc][HISTORY-1]));
		D(printf(" => curr-Signature: %3x\n", signature[assoc]));
	}
	else { // Read signature
		D(printf("\t[ST-READ] Index: %4x prev-History[%d]: ", idx, assoc));
		D(for (int i=0; i<HISTORY; i++))
			D(printf("%3d ", delta[assoc][i]));
		D(printf(" => prev-Signature: %3x\n", signature[assoc]));
	}
...
void LookaheadPrefetcher::SPP_class::update_PT(int tag)
{
	...
	for (i=0; i<PT_WAY; i++) {
		if (valid[i] && (p_tag[i] == tag)) { // Hit: Matching stride found
			LookaheadPrefetcher::SPP_class::update_counter(i, false);
			D(printf("\t[PT-UPDATE] Index: %4x  PT-Hit[%d]: %d  counter: %d\n",
						idx, i, tag, counter[i]));
			LookaheadPrefetcher::SPP_class::update_lru(i, false, FROM_PT);
			PT_hit++; PT_access++;
			return;
		}
			...
			p_tag[i] = tag;
			valid[i] = true;
			D(printf("\t[PT-UPDATE] Index: %4x  PT-Invalid[%d]: %d  counter: %d\n",
						idx, i, tag, counter[i]));
			LookaheadPrefetcher::SPP_class::update_lru(i, true, FROM_PT);
			PT_miss++; PT_access++;
			return;
		}
	...
	// Miss
	p_tag[match_idx] = tag;
	LookaheadPrefetcher::SPP_class::update_counter(match_idx, true);
	D(printf("\t[PT-UPDATE] Index: %4x  PT-Miss[%d]: %d does not exist Replacing counter[%d]: %d\n",
				idx, match_idx, tag, match_idx, counter[match_idx]));
	LookaheadPrefetcher::SPP_class::update_lru(match_idx, false, FROM_PT);
	PT_miss++; PT_access++;
	return;
}
...
int LookaheadPrefetcher::SPP_class::read_PT(int conf, int la_depth)
{
	...
	for (i=0; i<PT_WAY; i++) {
		...
		if (acc_conf >= PF_THRESHOLD) {
			...
			D(printf("\t[PT]  Index: %4x  Prefetch[%d]:  %3d  prev-conf: %3d  check-conf: %3d  la_depth: %2d\n",
...
//							idx, i, p_tag[i], conf, acc_conf, la_depth));
	...
	if (depth) // During lookahead, choose only one pf_candidate with maximum confidence
		pf_candidate = 1 << max_idx;

	if(la_candidate) {
		max_conf = MAX_CONFIDENCE*counter[max_idx]/(1 << COUNTER_BIT);
		D(printf("\t[PT] Index: %4x  Lookahead[%d]: %3d  has Local-MAX-conf: %d  la_depth: %2d\n",
...
void attempt_prefetch(Addr base_addr, int base_block, int PT_idx, int la_candidate, int pf_candidate)
{
	...
	D(printf("\t[PT-READ]  Lookahead: %d  Prefetch: %d  remain_mshr: %d  remain_rq: %d\n",
	...
	for (int i=0; i<PT_WAY; i++) {
		if (pf_candidate & (1 << i)) {
			...
			D(printf("\t[PT-READ]  Checking base_block: %llu  next-stride[%3x][%d]: %d  pf_block: %d\n",
						(base_addr >> 6) &0x3F, PT_idx, i, PT[PT_idx].p_tag[i], pf_block));

			if (pf_block < 0 || pf_block > 63) {
				PF_beyond++; PF_attempt++;
				D(printf("\t[PT-READ]  Out of same page boundary!\n"));
				continue;
			}
			...
			if ((remain_mshr > MSHR_THRESHOLD) && (remain_rq > RQ_THRESHOLD)) {
				...
				if ((remain_rq > RQ_THRESHOLD)) {
					...
					D(printf("\t[PREFETCH-LLC]   Not enough resource to prefetch!\n"));
					return;
				}
			}
...
void lookahead(Addr base_addr, int base_block, int prev_conf, int PT_idx, int la_candidate)
{
	...
	// DEBUG
	D(printf("\t[LOOKAHEAD] next-Signature: %3x => %3x  next_base_block: %3d\n", PT_idx, next_sig, next_base_block));

	if (next_base_block < 0 || next_base_block > 63) {
	D(printf("\t[LOOKAHEAD] Next base block is Out of same page boundary!\n"));
		return;
	}
...
bool LookaheadPrefetcher::SPP_class::check_filter(int tag, int block, int type)
{
	...
	for (int i=0; i<FILTER_WAY; i++) {
		...
		if (valid[i] && (p_tag[i] == tag)) {
			if (filter_bitmap[i][block]) { // filter_bitmap is already set
				switch (type) {
					case DEMAND:
						D(printf("\t[FILTER] Hit: tag: %2x Filtering (demand) request block: %2d type: %d return: %d\n",
									tag, block, type, false));
						return false;
						break;
					case PREFETCH:
						D(printf("\t[FILTER] Hit: tag: %2x Filtering (prefetch) request block: %2d type: %d return: %d\n",
									tag, block, type, false));
						return false;
						break;
					case FILL:
						D(printf("\t[FILTER] Hit: tag: %2x filter_bitmap is already on (fill) block: %2d type: %d return: %d\n",
									tag, block, type, false));
						return false;
						break;
					case EVICT:
						D(printf("\t[FILTER] Hit: tag: %2x Clear (evict) block: %2d type: %d return: %d\n",
									tag, block, type, true));
						filter_bitmap[i][block] = 0;
						return true;
						break;
					case CHECK:
						D(printf("\t[FILTER] Hit: tag: %2x Checked inflight pf_block: %2d type: %d return: %d\n",
									tag, block, type, false));
						return false;
						break;
				}
				...
				switch (type) {
					case DEMAND:
						D(printf("\t[FILTER] Hit: tag: %2x Set (demand) block: %2d type: %d return: %d\n",
									tag, block, type, true));
						filter_bitmap[i][block] = 1;
						return true;
						break;
					case PREFETCH:
						D(printf("\t[FILTER] Hit: tag: %2x Set (prefetch) block: %2d type: %d return: %d\n",
									tag, block, type, true));
						filter_bitmap[i][block] = 1;
						return true;
						break;
					case FILL:
						D(printf("\t[FILTER] Hit: tag: %2x Set (fill) block: %2d type: %d return: %d\n",
									tag, block, type, true));
						filter_bitmap[i][block] = 1;
						return false;
						break;
					case EVICT:
						...
						break;
					case CHECK:
						D(printf("\t[FILTER] Hit: tag: %2x Check no inflight pf_block: %2d type: %d return: %d\n",
									tag, block, type, true));
						return true;
						break;
				}
		...
		// Invalid
			...
			if (type == DEMAND) {
				filter_bitmap[i][block] = 1;
				D(printf("\t[FILTER] Invalid: tag: %2x (demand) recording block: %2d type: %d return: %d\n",
							tag, block, type, true));
			}
			else { // This is possible due to inclusive policy
				filter_bitmap[i][block] = 0;
				D(printf("\t[FILTER] Invalid: tag: %2x (evict) recording block: %2d type: %d return: %d\n",
							tag, block, type, true));
			}
			return true;
		}
	...
	if (type == DEMAND) {
		...
		D(printf("\t[FILTER] Miss: tag: %2x Demand recording block: %2d type: %d return: %d\n",
					tag, block, type, true));
		return true;
	}
